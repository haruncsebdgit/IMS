...
class IndicatorSetupRequest extends FormRequest
{
    /**
    ...
    public function rules()
    {
        return [
            'name_en' => [
                           "required",
                           "max:255",
                           Rule::unique('indicator_setups')
                                    ->where(function ($query) {
                                        return 
                                        $query->where('name_en', $this->name_en)
                                        ->where('indicator_category_id', $this->indicator_category_id)
                                        ->where('indicator_sub_category_id', $this->indicator_sub_category_id);
                                    })
                                    ->ignore($this->id)],
