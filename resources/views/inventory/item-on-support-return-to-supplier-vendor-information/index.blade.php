...
@section('body_class', 'Item On-Support or Return to Supplier/Vendor Information list')
...
<a class="btn btn-sm btn-primary" href="{{ action('Inventory\ItemOnSupportReturnToSupplierVendorInformationController@create') }}">
...
@if( ! $item_on_support_return_to_supplier_vendor_information->isEmpty() || filter_input(INPUT_GET, 'filter') )
@include ('inventory.item-on-support-return-to-supplier-vendor-information.search-form')
@endif

@include('errors.validation')

@if(!$item_on_support_return_to_supplier_vendor_information->isEmpty())
...
<div class="border mb-1">
    <table class="table table-hover table-striped mb-0">
        ...
        <tbody>
            @foreach($item_on_support_return_to_supplier_vendor_information as $item)
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>
                     @if( hasUserCap(['view_item_on_support_return_to_supplier_vendor_information']))
                        <a href="{{ action('Inventory\ItemOnSupportReturnToSupplierVendorInformationController@show', ['id' => $item->id]) }}">
                            {{ $item->id }}
                        </a>
                    @else
                        {{ $item->id }}
                ...
                <td>
                    <div class="dropdown">
                        ...
                        @if( hasUserCap(['edit_item_on_support_return_to_supplier_vendor_information','delete_item_on_support_return_to_supplier_vendor_information']) )
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="row-action-button-1">
                            ...
                            <a href="{{ action('Inventory\ItemOnSupportReturnToSupplierVendorInformationController@edit', ['id' => $item->id]) }}" class="dropdown-item">
                                <i class="icon-pencil7" aria-hidden="true"></i> {{ __('Edit') }}
                            </a>
                            @endif
                            @if( hasUserCap('view_item_on_support_return_to_supplier_vendor_information'))
                                <a href="{{ action('Inventory\ItemOnSupportReturnToSupplierVendorInformationController@show', ['id' =>  $item->id]) }}" class="dropdown-item">
                                    <i class="icon-eye8" aria-hidden="true"></i> {{ __('View') }}
                                </a>
                            @endif

                            @if( hasUserCap('delete_item_on_support_return_to_supplier_vendor_information') )
                            <div class="dropdown-divider"></div>

                            <form action="{{ route('item-on-support-return-to-supplier-vendor-information.delete', $item->id) }}" method="POST">
...
{!! gridFooter($item_on_support_return_to_supplier_vendor_information, $itemsPerPage) !!}
