...
$_filter_params = array(
    ...
    'inventory_center_from' => filter_input(INPUT_GET, 'inventory_center)from', FILTER_SANITIZE_STRING),
...
<div class="col-sm-3">
        <div class="form-group">
            <label for="name">{{ __('Receive Id') }}</label>
            {!! Form::number('receive_id', $_filter_params['receive_id'], ['class'=>'form-control' ]) !!}
        </div>
    </div>
    <div class="col-sm-3">
        <div class="form-group">
            <label for="registration_no">{{ __('Inventory Center From') }}</label>
